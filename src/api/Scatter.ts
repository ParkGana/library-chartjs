...
export const fetchScatterChartDataAPI = async (): Promise<ScatterChartDataType[]> => {
  const [resA, resB] = await Promise.all([
    axios.get(`${import.meta.env.VITE_API_URL}/scatter_A`),
    axios.get(`${import.meta.env.VITE_API_URL}/scatter_B`)
  ]);
  return [
...
export const fetchScatterChartRealtimeDataAPI = async (): Promise<ScatterChartDataType[]> => {
  const [resA, resB] = await Promise.all([
    axios.get(`${import.meta.env.VITE_API_URL}/scatter_realtime_A`),
    axios.get(`${import.meta.env.VITE_API_URL}/scatter_realtime_B`)
  ]);
  return [
    { name: 'A', data: resA.data },
    { name: 'B', data: resB.data }
...
export const createScatterChartRealtimeDataAPI = async (): Promise<void> => {
  await Promise.all([
    axios.post(`${import.meta.env.VITE_API_URL}/scatter_realtime_A`, {
      xvalue: faker.number.int({ min: 0, max: 100 }),
      yvalue: faker.number.int({ min: 0, max: 100 })
    }),
    axios.post(`${import.meta.env.VITE_API_URL}/scatter_realtime_B`, {
      xvalue: faker.number.int({ min: 0, max: 100 }),
      yvalue: faker.number.int({ min: 0, max: 100 })
    })
  ]);
